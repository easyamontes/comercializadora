...
class PedidoController extends Controller
{
    ...
    public function premio(Request $request)
    {
        ...
        if ($socio == "" and $inicio != '' and $final != '') {
            $pedido = Pedido::select('*')
                ...
                ->orderby('fechapedido')
                ->get();
        } else if ($socio != "" and $inicio == "" and $final == "") {
            $pedido = Pedido::select('*')
                ->where('user_id', '=', $per)
    ...
    /*======================================================================
       LISTA DE VENTAS PARA EL FONDO DE AHORRO
    ====================================================================== */
    public function listaahorro(Request $request)
    {
        ...
        $datefinal = str_replace('/', '-', $final);
        $datefinal = date('Y-m-d', strtotime($datefinal));
        if ( $inicio != '' and $final != ''and $socio == null) {
    ...
    /*======================================================================
         FUNCION DE PARA PAGAR FONDO DE AHORRO POR DIA
    ====================================================================== */
    public function pagar($id, Request $request)
    {
        ...
        unset($params_array['user']);
        unset($params_array['articulos']);
        $pedido = Pedido::where('fechapedido', $id)->update(['status' => 'PAGADO', 'fechapago' => $todayDate]);
        $data = array(
            'pedido' => $pedido,
            'code' => 200,
            'status' => 'success'
    ...
    ====================================================================== */
     public function pagartodo ($id,Request $request)
     {
         $json = $request->input('json',null);
         $params_array = json_decode($json, true);
         $todayDate = date("Y-m-d");  
                  foreach ($params_array as $item) {
             Pedido::where([['fechapedido','=',$item['fechapedido']],['user_id','=',$item['user_id']]])->update(['status' => 'PAGADO', 'fechapago' => $todayDate]);
          }
          $data = array(
            'pedido' => 'mensajea correcto',
            'code' => 200,
            'status' => 'success'
          );
          return response()->json($data, 200);
    ...
    public function listastatus(Request $request)
    {
        ...
        if ( $socio == '' and $inicio != '') {
            $pedido = Pedido::selectRaw('*')
                ...
                ->orderby('user_id')
                ->get();
        } else if ($socio != null and $inicio != null) {
            $pedido = Pedido::select('*')
                ->where('user_id', '=', $socio)->where('status', '=', $inicio)
                ->where('tipo', '=', 'ENTRADA')
                ->groupBy('user_id')
                ->groupBy('fechapedido')
                ->orderby('fechapedido')
